    ...
    def WriteValue(self, value, options):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Base64 string: '{base64_str}'")
                ...
                logger.warning(f"Base64 decode failed: {decode_error}. Trying direct UTF-8 decode.")
                # フォールバック: 直接UTF-8デコード試行
                command_str = bytes(value).decode('utf-8').strip()
                logger.info(f"Fallback UTF-8 decode: '{command_str}'")
            ...
            # コマンドごとの処理
            if command_str == "RUN":
                logger.info("RUN command received - Starting motors")
                # I2CでArduinoに送信
                if bus:
                    try:
                        data_bytes = [ord(char) for char in command_str]
                        bus.write_i2c_block_data(ARDUINO_I2C_ADDRESS, 0, data_bytes)
                        logger.info(f"Sent RUN command to Arduino via I2C")
                        GLib.idle_add(send_status_notification, "CMD_OK:RUN")
                    except Exception as i2c_error:
                        logger.error(f"I2C write error: {i2c_error}")
                        GLib.idle_add(send_status_notification, "ERR:I2C_RUN")
                else:
                    logger.warning("I2C bus not initialized")
                    GLib.idle_add(send_status_notification, "ERR:I2C_Not_Ready")
                    
            elif command_str == "STOP":
                logger.info("STOP command received - Stopping motors")
                # I2CでArduinoに送信
                if bus:
                    try:
                        data_bytes = [ord(char) for char in command_str]
                        bus.write_i2c_block_data(ARDUINO_I2C_ADDRESS, 0, data_bytes)
                        logger.info(f"Sent STOP command to Arduino via I2C")
                        GLib.idle_add(send_status_notification, "CMD_OK:STOP")
                    except Exception as i2c_error:
                        logger.error(f"I2C write error: {i2c_error}")
                        GLib.idle_add(send_status_notification, "ERR:I2C_STOP")
                else:
                    logger.warning("I2C bus not initialized")
                    GLib.idle_add(send_status_notification, "ERR:I2C_Not_Ready")
                    
            elif command_str in ["UP", "DOWN", "LEFT", "RIGHT", "FWD", "BACK", "PARALEL"]:
                logger.info(f"Direction command received: {command_str}")
                # I2CでArduinoに送信
                if bus:
                    try:
                        data_bytes = [ord(char) for char in command_str]
                        bus.write_i2c_block_data(ARDUINO_I2C_ADDRESS, 0, data_bytes)
                        logger.info(f"Sent direction command to Arduino via I2C: {command_str}")
                        GLib.idle_add(send_status_notification, f"CMD_OK:{command_str}")
                    except Exception as i2c_error:
                        logger.error(f"I2C write error: {i2c_error}")
                        GLib.idle_add(send_status_notification, f"ERR:I2C_{command_str}")
                else:
                    logger.warning("I2C bus not initialized")
                    GLib.idle_add(send_status_notification, "ERR:I2C_Not_Ready")
                    
            elif command_str.startswith("PID_"):
                logger.info(f"PID command received: {command_str}")
                # I2CでArduinoに送信
                if bus:
                    try:
                        data_bytes = [ord(char) for char in command_str]
                        bus.write_i2c_block_data(ARDUINO_I2C_ADDRESS, 0, data_bytes)
                        logger.info(f"Sent PID command to Arduino via I2C: {command_str}")
                        GLib.idle_add(send_status_notification, f"PID_SET:{command_str[4:14]}")
                    except Exception as i2c_error:
                        logger.error(f"I2C write error: {i2c_error}")
                        GLib.idle_add(send_status_notification, "ERR:I2C_PID")
                else:
                    logger.warning("I2C bus not initialized")
                    GLib.idle_add(send_status_notification, "ERR:I2C_Not_Ready")
                    
            elif command_str == "PID_ON" or command_str == "PID_OFF":
                logger.info(f"PID control command received: {command_str}")
                # I2CでArduinoに送信
                if bus:
                    try:
                        data_bytes = [ord(char) for char in command_str]
                        bus.write_i2c_block_data(ARDUINO_I2C_ADDRESS, 0, data_bytes)
                        logger.info(f"Sent PID control command to Arduino via I2C: {command_str}")
                        GLib.idle_add(send_status_notification, f"CMD_OK:{command_str}")
                    except Exception as i2c_error:
                        logger.error(f"I2C write error: {i2c_error}")
                        GLib.idle_add(send_status_notification, "ERR:I2C_PID_CTRL")
                else:
                    logger.warning("I2C bus not initialized")
                    GLib.idle_add(send_status_notification, "ERR:I2C_Not_Ready")
                    
            elif command_str.startswith("TEST"):
                logger.info(f"TEST command received: {command_str}")
                # I2CでArduinoに送信
                if bus:
                    try:
                        data_bytes = [ord(char) for char in command_str]
                        bus.write_i2c_block_data(ARDUINO_I2C_ADDRESS, 0, data_bytes)
                        logger.info(f"Sent TEST command to Arduino via I2C: {command_str}")
                        GLib.idle_add(send_status_notification, f"TEST_OK:{command_str}")
                    except Exception as i2c_error:
                        logger.error(f"I2C write error: {i2c_error}")
                        GLib.idle_add(send_status_notification, "ERR:I2C_TEST")
                else:
                    logger.warning("I2C bus not initialized")
                    GLib.idle_add(send_status_notification, "ERR:I2C_Not_Ready")
                    
            elif command_str == "STATUS":
                logger.info("STATUS command received - Requesting status from Arduino")
                # I2CでArduinoに送信して、ステータスを要求
                if bus:
                    try:
                        data_bytes = [ord(char) for char in command_str]
                        bus.write_i2c_block_data(ARDUINO_I2C_ADDRESS, 0, data_bytes)
                        logger.info(f"Sent STATUS request to Arduino via I2C")
                        # ここでArduinoからの返信を読み取る処理を追加することも可能
                        GLib.idle_add(send_status_notification, "STATUS_REQ_SENT")
                    except Exception as i2c_error:
                        logger.error(f"I2C write error: {i2c_error}")
                        GLib.idle_add(send_status_notification, "ERR:I2C_STATUS")
                else:
                    logger.warning("I2C bus not initialized")
                    GLib.idle_add(send_status_notification, "ERR:I2C_Not_Ready")
                ...
                # それでも未知のコマンドはArduinoに転送
                if bus:
                    try:
                        data_bytes = [ord(char) for char in command_str]
                        bus.write_i2c_block_data(ARDUINO_I2C_ADDRESS, 0, data_bytes)
                        logger.info(f"Sent unknown command to Arduino via I2C: {command_str}")
                        GLib.idle_add(send_status_notification, f"CMD_FWD:{command_str[:13]}")
                    except Exception as i2c_error:
                        logger.error(f"I2C write error: {i2c_error}")
